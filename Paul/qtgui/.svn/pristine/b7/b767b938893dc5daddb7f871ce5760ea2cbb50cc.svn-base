...
class FormControllerEdit(QtGui.QDialog, Ui_dialog_control_edit):
  def __init__(self, parent):
    ...
    #self.buttonBox.button(QtGui.QDialogButtonBox.Retry).clicked.connect(self.button_retry_clicked)
...
class UControlUnit(object):
  ...
  def init(self):
    ...
    self.editDlg.buttonBox.button(QtGui.QDialogButtonBox.Apply).clicked.connect(self.configuration_to_robot)
    self.editDlg.buttonBox.button(QtGui.QDialogButtonBox.Cancel).clicked.connect(self.buttonCancelClicked)
    self.editDlg.buttonBox.button(QtGui.QDialogButtonBox.Ok).clicked.connect(self.buttonOkClicked)
  ...
  def button_retry_clicked(self):
    self.robot.conWrite("x" + self.ctrlID + "\n")
    print("Send: " + "x" + self.ctrlID + "\n")
    #self.fillDialog()
    #print("#filled dialog")
    pass
  ...
  def asString(self):
    if self.robot.info.talkToBridge:
      ...
      s = ""
    s += (self.ctrlID + " {:d} {:g}".format(self.use, self.Kp) + self.integrator.asString() + self.leadFwd.asString() +
        self.leadBack.asString() + self.preFilt.asString() + self.postFiltI.asString() +
        " {:d} {:g}".format(self.ffUse, self.ffKp) + self.ffFilt.asString() + 
        " {:d} {:g}\n".format(self.outLimitUse, self.outLimit))
    return s
  ...
  def configuration_to_robot(self):
    self.setFromDialog()
    self.robot.conWrite(self.asString() + "\n")
    print("send data")
    pass
  ...
  def requestDataFromRobot(self):
    if self.robot.info.talkToBridge:
      ...
      self.robot.conWrite(self.ctrlID + " subscribe 6\n")
      self.robot.conWrite("robot x" + self.ctrlID + "\n")
    else:
      self.robot.conWrite("x" + self.ctrlID + "\n")
