...
# "$Rev: 959 $"
#CLIENT_REV = "$Id: regbot.py 959 2018-10-19 15:31:06Z jcan $"
...
class UMainWindow(QtGui.QMainWindow):
  def __init__(self, parent = None):
    ...
    #
    p = self.ui.Encoder.palette() # encoder
    p.setColor(self.ui.Encoder.backgroundRole(), QtCore.Qt.lightGray)
    self.ui.Encoder.setPalette(p)
    self.ui.Encoder.setAutoFillBackground(True)
    #
    ...
    self.ui.actionEncoder.triggered.connect(self.menuShowEncoder)
    ...
    #self.ui.ls_crossing_detect.valueChanged.connect(self.robot.lineSensor.setWhiteLine)
    #self.ui.ls_wideSensor.clicked.connect(self.robot.lineSensor.setWhiteLine)
    self.ui.ls_apply.clicked.connect(self.robot.lineSensor.butApply) 
    self.ui.ls_cancel.clicked.connect(self.robot.lineSensor.butCancel)
    self.ui.ls_edit.clicked.connect(self.robot.lineSensor.butEdit)
    # main status
    ...
    # encoder
    self.ui.encoder_do_calibrate.clicked.connect(self.robot.encoder.doCalibrate)
    self.ui.encoder_reindex.clicked.connect(self.robot.encoder.reindex)
    self.ui.encoder_run_motors.clicked.connect(self.robot.encoder.runMotors)
    self.ui.encoder_collect.clicked.connect(self.robot.encoder.collectTiming)
    self.ui.encoder_use_calibration.clicked.connect(self.robot.encoder.calibrationUse)
    self.ui.encoder_auto_calibrate.clicked.connect(self.robot.encoder.autoCalibrate)
    #log 
    ...
    self.ui.log_ctrl_bal_pos.stateChanged.connect(self.robot.setLogFlagControl)
    ...
    self.ui.pushButton_ctrlBalPos.clicked.connect(self.robot.ctrlBalPos.editControlValues)
    # wifi
  ...
  def getIndex(self, tab):
    ...
    if (tab <= 4):
      return i
    if (self.ui.actionEncoder.isChecked()):
      i = i + 1
    if (tab <= 5):
      return i
  ...
  def menuShowMission(self):
    if (self.ui.actionMission.isChecked()):
      self.ui.tabPages.insertTab(self.getIndex(10), self.ui.tab_6, "Mission")
  ...
  def menuShowServo(self):
    if (self.ui.actionServo.isChecked()):
      self.ui.tabPages.insertTab(self.getIndex(9), self.ui.tab_servo, "Servo")
  ...
  def menuShowControl(self):
    if (self.ui.actionControl.isChecked()):
      self.ui.tabPages.insertTab(self.getIndex(8), self.ui.tab_control, "Control")
  ...
  def menuShowDist(self):
    if (self.ui.actionDistance.isChecked()):
      self.ui.tabPages.insertTab(self.getIndex(7), self.ui.tab_11, "IR Dist")
  ...
  def menuShowLine(self):
    if (self.ui.actionLine.isChecked()):
      self.ui.tabPages.insertTab(self.getIndex(6), self.ui.tab_10, "Edge")
  ...
  def menuShowIMU(self):
    if (self.ui.actionIMU.isChecked()):
      self.ui.tabPages.insertTab(self.getIndex(5), self.ui.tab_3, "IMU")
      ...
      if t >= 0:
        self.ui.tabPages.removeTab(t)
    pass
  def menuShowEncoder(self):
    if (self.ui.actionEncoder.isChecked()):
      self.ui.tabPages.insertTab(self.getIndex(4), self.ui.Encoder, "Encoder")
      self.ui.tabPages.setCurrentIndex(self.getIndex(4))
    else:
      t = self.ui.tabPages.indexOf(self.ui.Encoder)
      if t >= 0:
        self.ui.tabPages.removeTab(t)
    pass
  ...
  def menuShowAll(self):
    ...
    if not self.ui.actionRobot.isChecked():
      ...
      self.menuShowRobot()
    if not self.ui.actionEncoder.isChecked():
      self.ui.actionEncoder.setChecked(True)
      self.menuShowEncoder()
  ...
  def menuHideMost(self):
    ...
    if not self.ui.actionRobot.isChecked():
      ...
      self.menuShowRobot()
    if not self.ui.actionEncoder.isChecked():
      self.ui.actionEncoder.setChecked(False)
      self.menuShowEncoder()
    if self.ui.actionIMU.isChecked():
  ...
  def tabPageFocusChanged(self):
    ...
    elif (tab == self.ui.tabPages.indexOf(self.ui.Encoder)):
      # request data to ENCODER CALIBRATION
      self.robot.conWrite("u25\n")
      print("# requested encoder timing values >>u25")
      self.robot.currentTab = "encoder"
