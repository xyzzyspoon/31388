...
class ControlPaintSpace(PaintSupport):
  ...
  def paintEvent(self, QPaintEvent):
    ...
    #paint.setBrush(QtCore.Qt.NoBrush)
    #print("control paint")
    p1Mix = QtCore.QPoint(self.ui.frame_mix_select.x() + self.ui.frame_mix_select.width(), self.ui.frame_mix_select.y() + self.ui.frame_mix_select.height() - 150);
    p1Vel = QtCore.QPoint(self.ui.frame_ctrl_vel.x() + self.ui.frame_ctrl_vel.width()/2, self.ui.frame_ctrl_vel.y());
    ...
    #
    p2WallDist = QtCore.QPoint(self.ui.frame_ctrlWallVel.x() + self.ui.frame_ctrlWallVel.width(), self.ui.frame_ctrlWallVel.y() + self.ui.frame_ctrlWallVel.height()/2); # balance ctrl
    ...
    #
    p2WallTurn = QtCore.QPoint(self.ui.frame_ctrlWallTurn.x() + self.ui.frame_ctrlWallTurn.width(), self.ui.frame_ctrlWallTurn.y() + self.ui.frame_ctrlWallTurn.height()/2); # balance ctrl
    ...
    #
    p2Edge = QtCore.QPoint(self.ui.frame_ctrlEdge.x() + self.ui.frame_ctrlEdge.width(), self.ui.frame_ctrlEdge.y() + self.ui.frame_ctrlEdge.height()/2); # edge ctrl
    paint.drawLine(p2Edge, QtCore.QPoint(p1MixX, p2Edge.y())) # from line edge to mix
    self.paintArrow(paint, p1MixX, p2Edge.y(), 0, col)
    #
    p2Pos = QtCore.QPoint(self.ui.frame_ctrlPos.x() + self.ui.frame_ctrlPos.width(), self.ui.frame_ctrlPos.y() + self.ui.frame_ctrlPos.height()/2); # position ctrl
    paint.drawLine(p2Pos, QtCore.QPoint(p1MixX, p2Pos.y())) # from tilt to mix
    self.paintArrow(paint, p1MixX, p2Pos.y(), 0, col)
    #
    p2Turn = QtCore.QPoint(self.ui.frame_turn.x() + self.ui.frame_turn.width(), self.ui.frame_turn.y() + self.ui.frame_turn.height()/2); # turn ctrl
    paint.drawLine(p2Turn, QtCore.QPoint(p1MixX, p2Turn.y())) # from turn to mix
    self.paintArrow(paint, p1MixX, p2Turn.y(), 0, col)
    #
    p2BalVel = QtCore.QPoint(self.ui.frame_ctrlBalVel.x() + self.ui.frame_ctrlBalVel.width(), self.ui.frame_ctrlBalVel.y() + self.ui.frame_ctrlBalVel.height()/2); # balance velocity ctrl
    p2BalPos = QtCore.QPoint(self.ui.frame_ctrlBalPos.x() + self.ui.frame_ctrlBalPos.width(), self.ui.frame_ctrlBalPos.y() + self.ui.frame_ctrlBalPos.height()/2); # balance position ctrl
    ...
    p1MisMixY = (p2BalVel.y() + p2WallDist.y())/2
    ...
    p2MisX = self.ui.frame_mission_ctrl.x() + self.ui.frame_mission_ctrl.width()/2
    ...
    p2PosX = self.ui.frame_ctrlPos.x() + self.ui.frame_ctrlPos.width()/2
    ...
    p2BalVelX = self.ui.frame_ctrlBalVel.x() + self.ui.frame_ctrlBalVel.width()/2
    paint.drawLine(QtCore.QPoint(p2BalVelX, p1SenY), QtCore.QPoint(p2BalVelX, p2BalVelY))
    self.paintArrow(paint, p2BalVelX, p2BalVelY, 1, col)
    # from sensor to balance position
    p2BalPosX = self.ui.frame_ctrlBalPos.x() + self.ui.frame_ctrlBalPos.width()/2
    ...
    p2TurnX = self.ui.frame_turn.x() + self.ui.frame_turn.width()/2
