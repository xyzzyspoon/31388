/*
This program tracks orange balls in real time using colour recognition and computes its width.
The program takes in live video feed in RGB. Then converts it into the HSV format.
Using the threshold values mentioned for orange, it forms a binary image.
The objects that we are detecting are white and everything else is black.
Then, we use a "Bitwise And" operation - we multiply the binary image by our original image to obtain another view where we just visualise the orange ball.
Lastly we draw a bounding box for our ball and label it and mention its width.
*/
...
char ball_window[] = "The Watcher Detected the Ball";
...
void orange_ball(Mat live_image, Mat live_image_HSV); // Function to detect orange objects
float compute_ppm(int box_width); // Computes the pixel per metric ratio of the image
...
void live_video(Mat live_image, Mat live_image_HSV)
{
	...
	while (1)
	{
		...
		orange_ball(live_image, live_image_HSV);
...
void orange_ball(Mat live_image, Mat live_image_HSV)
{
	// PART 1 - OBJECT DETECTION

	Mat live_mask_orange; // Stores the binary mask for the orange object
	Mat orange_ball; // Stores the final orange object that we detected

	// Lower and upper thresholds for the orange objects
	// MODIFY AFTER EVERY CALIBRATION
	Scalar orange_lower = Scalar(115, 21, 115);
	...
	bitwise_and(live_image, live_image, orange_ball, live_mask_orange); // Bitwise AND operation will show only the orange object (if accurately calibrated)
	imshow(ball_window, orange_ball);
	...
	string my_text = "Orange Ball"; // The text that we want to use for our label
	...
	for (int i = 0; i < live_image_contours.size(); i++)
	{
		...
		double threshold_contour_area = 300; // We want to draw bounding boxes for contours with a minimum area only. This can help us remove noise

		if (orange_objects_area > threshold_contour_area)
		{
			cout << "But now, by telling you about the orange ball I am breaking my most sacred vow." << endl;
			cout << "Know all of you that Mia is approaching an orange ball." << endl;
			...
			Scalar br_colour = Scalar(180, 150, 255); // The bottom right corner is orange
			...
			float pixel_per_metric = compute_ppm(box_width); // Computing the pixel per metric ratio of the image

			cout << "PPM = " << pixel_per_metric << endl;

			float computed_width = box_width / pixel_per_metric;

			putText(live_image, to_string(computed_width), bounding_rectangle[i].br(), font, font_face, text_colour, text_thickness, line_type, bottom_left_origin);
			// tl() - top left corner
			// br() - top right corner

			cout << "Ball diameter = " << computed_width << " mm" << endl;
...
float compute_ppm(int box_width)
{	
	float pixel_per_metric = 0;
	float known_width = 40.0; // Width of the ball that we are measuring
	pixel_per_metric = box_width / known_width;
